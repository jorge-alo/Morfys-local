...
export const FormModalLocal = ({ userId, setShowModalLocal }) => {
    ...
    const { handleClickSend, loading, handleClose } = useClickSend(userId, setShowModalLocal)
    return (
        <form className='modalLocal-container__form-ajustes' onSubmit={handleClickSend}>
            ...
            <button disabled={loading} className='button-ajustes' >{loading ? 'Cargando...' : 'enviar'}</button>
        </form>
    )
}
